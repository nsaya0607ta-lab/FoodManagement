  ...
  return (
    <div className="flex flex-col gap-4 px-4 pt-5">
      ...
      {!db ? (
        ...
        <StorageAreaList areas={areas} />
      )}
    </div>
  );
}

function StorageAreaList({ areas }: { areas: StorageArea[] }) {
    ...
    <div className="flex flex-col gap-2">
      {areas.map((area) => (
        ...
        >
          ...
          <span className="text-sm text-zinc-500 dark:text-zinc-400">{counts[area.id] ?? 0}品</span>
        </Link>
      ))}
    </div>
  );
